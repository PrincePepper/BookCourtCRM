...
from ..models.books import BookCreate, BookUpdate, BookDesctiption
...
class BooksService:
    ...
    def get_desciption(self, company_id: int):
        # return self.session.execute("select description from books;")
        return self.session.query(tables.Book.description).filter(tables.Book.company_id == company_id).all()

    def get_ids(self, company_id: int):
        # return self.session.execute("select description from books;")
        return self.session.query(tables.Book.id).filter(tables.Book.company_id == company_id).all()
